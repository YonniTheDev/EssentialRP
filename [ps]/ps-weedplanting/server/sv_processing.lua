...
RegisterNetEvent('ps-weedplanting:server:ProcessBranch', function()
    ...
    if item and item.info.health then
        if Player.Functions.RemoveItem(Shared.BranchItem, 50, item.slot) then
            TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items[Shared.BranchItem], 'remove', 50)
