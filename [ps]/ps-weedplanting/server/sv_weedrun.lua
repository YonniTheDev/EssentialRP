...
QBCore.Functions.CreateCallback('ps-weedplanting:server:PackageGoods', function(source, cb)
    ...
    if Player.Functions.RemoveItem(Shared.PackedWeedItem, 50) then
        TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items[Shared.PackedWeedItem], 'remove', 50)
    else
        cb(false)
        return
    end
