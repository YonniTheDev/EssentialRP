...
-- Events
RegisterNetEvent('essential-jobgarages:client:newVehicleSetup', function(job, grades, categories, random_plate)
     local function saveVehicle(d, veh)
          ...
          TriggerCallback('essential-jobgarages:server:save_vehicle', function(res)
     ...
     if not IsPedInAnyVehicle(plyPed, false) then
          TriggerServerEvent('essential-jobgarages:server:Notification', 'You should be inside a vehicle to use this command', 'error')
          return
     end

     local veh = GetVehiclePedIsIn(plyPed, false)
     if not inGarageStation then
          TriggerServerEvent('essential-jobgarages:server:Notification', 'You must be in garage zone!', 'error')
          return
     end
                    ...
                    if CanPlayerUseGarage() then
                         ...
                         if garage and garage.type == 'job' then
                              ...
                              radialmenu = exports['qb-radialmenu']:AddOption({
                                   ...
                                   icon = 'warehouse',
                                   type = 'client',
                                   event = 'essential-jobgarages:client:keep_put_back_to_garage',
                                   shouldClose = true
                              })
                              exports['qb-core']:DrawText('Police Garage')
                         elseif garage and garage.type == 'gang' then
                              if radialmenu then
                                   exports['qb-radialmenu']:RemoveOption(radialmenu)
                                   radialmenu = nil
...
RegisterNetEvent('essential-jobgarages:client:get_current_garage', function(event, data)
