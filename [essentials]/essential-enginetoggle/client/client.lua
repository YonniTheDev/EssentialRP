RegisterNetEvent('essential:Engine')
RegisterNetEvent('essential:RPDamage')
    ...
    while true do
        ...
        if UseKey and ToggleKey then
            if IsControlJustReleased(1, ToggleKey) then
                TriggerEvent('essential:Engine')
                Citizen.Wait(800)
            end
        end
        if GetSeatPedIsTryingToEnter(GetPlayerPed(-1)) == -1 or IsPedInAnyVehicle(GetPlayerPed(-1), false) then
...
AddEventHandler('essential:Engine', function()
    local veh
    local StateIndex
    for i, vehicle in ipairs(vehicles) do
...
AddEventHandler('essential:RPDamage', function(State)
    RPWorking = State
end)
