...
RegisterServerEvent('essential-runs:server:SetOccupied', function(route)
	Config.Routes[route].info.occupied = true
	TriggerClientEvent('essential-runs:client:SetOccupied', -1, route)
end)

-- Starting Conditions
RegisterServerEvent('essential-runs:server:startr', function()
    local src = source
	local Player = QBCore.Functions.GetPlayer(src)

	if Player.PlayerData.money['cash'] >= Config.RunCost then
		TriggerClientEvent("essential-runs:client:sendToOxy", src)
	else
		TriggerClientEvent('QBCore:Notify', source, Lang:t("error.you_dont_have_enough_money"), 'error')
	end
end)

RegisterServerEvent('essential-runs:server:TakeMoney', function()
    local src = source
	local Player = QBCore.Functions.GetPlayer(src)
	Player.Functions.RemoveMoney('cash', Config.RunCost)

	if Config.EnableCooldown then
        TriggerEvent('essential-runs:server:coolout')
	end
end)

-- Cooldown
RegisterServerEvent('essential-runs:server:coolout', function()
    Cooldown = true
...
QBCore.Functions.CreateCallback("essential-runs:server:coolc",function(source, cb)
    if Cooldown then
        cb(true)
    else
        cb(false)
...
QBCore.Functions.CreateCallback('essential-runs:server:getCops', function(source, cb)
    local amount = 0
...
RegisterNetEvent("essential-runs:server:sendCar")
AddEventHandler("essential-runs:server:sendCar", function(Routes)
	if oxyStarted == false then
		TriggerClientEvent("essential-runs:createOxyVehicle", source, Routes)
	end
end)

RegisterNetEvent("essential-runs:server:deliver")
AddEventHandler("essential-runs:server:deliver", function()
...
QBCore.Functions.CreateUseableItem('package', function(source)
	TriggerClientEvent('essential-runs:client:hasPackage', source) 
end)


QBCore.Functions.CreateCallback('essential-runs:server:hasPackage', function(source, cb)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
	local hasPackage = Player.Functions.GetItemByName("package")
...
QBCore.Functions.CreateCallback('essential-runs:server:canCarry', function(source, cb)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
