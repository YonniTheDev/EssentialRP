...
function hacksuccess()
    ...
    TriggerServerEvent('essential-atms:server:timer')

    if Config.MoneyType == true then

	    TriggerServerEvent("essential-atms:server:success")

    else

        TriggerServerEvent("essential-atms:server:success2")
...
function hackfailed(data)
    ...
    TriggerServerEvent('essential-atms:server:giveitemback')
...
---------------------------------------------------------------------------------------# Events #---------------------------------------------------------------------------------------
RegisterNetEvent('essential-atms:robatm', function()
    if Config.Framework == 'QBCore' or 'OX' then
        
        if CurrentCops >= Config.Cops then

            QBCore.Functions.TriggerCallback("essential-atms:server:Cooldown",function(isCooldown)
                if not isCooldown then

                    TriggerEvent('animations:client:EmoteCommandStart', {"parkingmeter"})
                    TriggerServerEvent('essential-atms:server:removeitem')
