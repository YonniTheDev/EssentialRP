...
-------------------------------------------
------ ONLY USED FOR essential-prisonJOBS --------
-------------------------------------------
	...
	if not targetsCreated then
		CheckTime = exports['qb-target']:AddBoxZone("prisontime", vector3(1827.3, 2587.72, 46.01), 0.4, 0.55, {
			...
			maxZ = 47.01,
				...
				{
					type = "client",
					event = "essential-prison:client:checkTime",
					icon = "fas fa-clock",
					label = "Check Jail Time",
				},
				{
					type = "client",
					event = "essential-prison:client:jobMenu",
		...
		for k,v in pairs(Config.Locations.PrisonBreak) do
			exports['qb-target']:AddBoxZone("prisonbreak"..k, vector3(v.coords.x, v.coords.y, v.coords.z), v.length, v.width, {
				...
				maxZ = v.coords.z + 1,
					...
					{
						type = "client",
						event = "essential-prison:StartPrisonBreak",
		...
		Canteen = exports['qb-target']:AddBoxZone("prisoncanteen", vector3(1780.95, 2560.05, 45.67), 3.8, 0.5, {
			...
			maxZ = 45.85,
				...
				{
					type = "client",
					event = "essential-prison:client:useCanteen",
		...
		Slushy = exports['qb-target']:AddBoxZone("prisonslushy", vector3(1777.64, 2559.97, 45.67), 0.5, 0.7, {
			...
			maxZ = 46.75,
				...
				{
					type = "client",
					event = "essential-prison:client:slushy",
		...
		Coffee = exports['qb-target']:AddBoxZone("prisoncoffee", vector3(1778.83, 2560.04, 45.67), 0.5, 0.3, {
			...
			maxZ = 46.75,
				...
				{
					type = "client",
					event = "essential-prison:client:coffee",
					icon = "fas fa-mug-hot",
					label = "Make Coffee",
				},
			},
		...
		Soda = exports['qb-target']:AddBoxZone("prisonsoda", vector3(1778.26, 2560.02, 45.67), 0.6, 0.55, {
			...
			maxZ = 46.75,
				...
				{
					type = "client",
					event = "essential-prison:client:soda",
					icon = "fas fa-cash-register",
					label = "Make Soda",
				},
			},
		...
		if Config.Crafting then
			Crafting = exports['qb-target']:AddBoxZone("prisoncrafting", vector3(Config.CraftingLocation.x, Config.CraftingLocation.y, Config.CraftingLocation.z), 1.4, 0.5, {
				...
				maxZ = Config.CraftingLocation.z + 1,
					...
					{
						type = "client",
						event = "essential-prison:CraftingMenu",
						icon = "fas fa-toolbox",
		...
		if not Config.QB_PrisonJobs then
			exports['qb-target']:AddBoxZone("PrisonChinUp", Config.Workout.Chinup.coords, 1, 2.0, {
				...
				options = {
					{
						type = "client",
						event = "essential-prison:client:DoChinUp",
...
RegisterNetEvent('QBCore:Client:OnPlayerLoaded', function()
	...
	TriggerEvent('essential-prison:client:SpawnLockers')
	PlayerJob = QBCore.Functions.GetPlayerData().job
end)
...
RegisterNetEvent('QBCore:Client:OnPlayerUnload', function()
	...
	DestroyAllTargets()
	TriggerEvent('essential-prison:client:RemoveLockers')
end)
...
-- Job Menu
RegisterNetEvent('essential-prison:client:jobMenu', function()
    if inJail then
		...
		pjobMenu = {
			...
			},
			{
				...
				params = {
					isServer = false,
					event = "essential-prison:jobapplyElectrician",
				}
			},
			{
				header = "Cook",
				...
				params = {
					isServer = false,
					event = "essential-prison:jobapplyCook",
				}
			},
			{
				header = "Janitor",
				...
				params = {
					isServer = false,
					event = "essential-prison:jobapplyJanitor",
				}
			},
			{
				header = "Close Menu",
...
RegisterNetEvent('essential-prison:jobapplyElectrician', function(args)
    ...
    if dist < 2 then
        ...
        if inJail then
			...
			TriggerEvent('essential-prison:client:jobMenu')
		end
	else
...
RegisterNetEvent('essential-prison:jobapplyCook', function(args)
	local pos = GetEntityCoords(PlayerPedId())
	inRange = false
...
RegisterNetEvent('essential-prison:jobapplyJanitor', function(args)
	local pos = GetEntityCoords(PlayerPedId())
	inRange = false
...
-- Check Time
RegisterNetEvent('essential-prison:client:checkTime', function()
	if inJail then
		local pos = GetEntityCoords(PlayerPedId())
...
------------------------------------
-- Use Canteen
RegisterNetEvent('essential-prison:client:useCanteen', function()
	if inJail then
		local ShopItems = {}
...
-- Slushy Machine
RegisterNetEvent('essential-prison:client:slushy', function()
	if inJail then
		Wait(1000)
...
-- Soda Machine
RegisterNetEvent('essential-prison:client:soda', function()
	if inJail then
		Wait(1000)
		local ped = PlayerPedId()
...
RegisterNetEvent('essential-prison:client:coffee', function()
	if inJail then
		local CoffeeItems = {}
...
RegisterNetEvent('essential-prison:client:Slushy', function(data)
...
RegisterNetEvent('essential-prison:CraftingMenu', function()
	...
	for k, v in pairs(Config.CraftingItems) do
		...
		item.icon = v.receive
		item.params = {
			event = 'essential-prison:CraftItem',
			args = {
			item = k
			}
		}
...
local function CraftItem(item)
	...
	QBCore.Functions.Progressbar('crafting_item', 'Crafting '..sharedItems[craftedItem].label, 5000, false, false, {
		...
		TriggerServerEvent('essential-prison:server:GetCraftedItem', Config.CraftingItems[item].receive)
		TriggerServerEvent('essential-prison:server:RemoveCraftingItems', item)
		TriggerEvent('animations:client:EmoteCommandStart', {"c"})
		ClearPedTasks(PlayerPedId())
	end, function()
		ClearPedTasks(PlayerPedId())
...
RegisterNetEvent('essential-prison:CraftItem', function(data)
	QBCore.Functions.TriggerCallback("essential-prison:server:CraftingMaterials", function(hasMaterials)
