...
RegisterNetEvent('essential-prison:server:getCommissary', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('essential-prison:server:GetCraftingItems', function(item, amount)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
-- Get Crafted Item
RegisterNetEvent('essential-prison:server:GetCraftedItem', function(data)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local item = data
...
RegisterNetEvent('essential-prison:server:RemoveCraftingItems', function(item)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback('essential-prison:server:CraftingMaterials', function(source, cb, materials)
    local src = source
...
QBCore.Functions.CreateUseableItem("slushy", function(source, item)
	...
	if Player.Functions.RemoveItem(item.name, 1, item.slot) then
        TriggerClientEvent("essential-prison:client:Slushy", src, item.name)
    end
end)
...
QBCore.Functions.CreateCallback('essential-prison:server:DoesStashExist', function(source, cb, stashID)
    local retval = false
