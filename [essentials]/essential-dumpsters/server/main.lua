...
RegisterNetEvent('essential-dumpsters:server:ResetEntity', function(entity)
    entities[entity] = 0
end)
...
RegisterNetEvent('essential-dumpsters:server:SetEntity', function(netId, isFinished)
    entities[netId] = 0
...
local function pGive(playerId, item, amount)
    ...
    if type(item) == 'string' then
        TriggerClientEvent('inventory:client:ItemBox', source,  QBCore.Shared.Items[i.item].label)
...
QBCore.Functions.CreateCallback('essential-dumpsters:server:getEntityState', function(source, cb, netId)
...
AddEventHandler('onResourceStop', function(resName)
    ...
    for _,v in pairs(entities) do
        if v == -1 then
            TriggerClientEvent('essential-dumpsters:client:ResetEntity', -1, _)
        end
    end
end)