...
elseif Config.Minigame and Config.Minigame ~= 'none' then
    print('[essential-dumpsters] Invalid minigame specified in config.lua')
end
...
function PoliceAlert()
    if Config.Illegal then
        ...
        elseif Config.Dispatch == 'qb-core' then
            ...
            print('[essential-dumpsters] Config.Dispatch does not have a valid argument')
        end
    end
end
...
function ProgressBar(ent)
    QBCore.Functions.Progressbar('diving_in_dumpster', Lang:t('progress.diving'), Config.ProgressTime * 1000, false, true, {
        ...
        TriggerServerEvent('essential-dumpsters:server:SetEntity', ObjToNet(ent), true)
        if not Config.ResetOnReboot then TriggerServerEvent('essential-dumpsters:server:ResetEntity', ObjToNet(ent)) end
        isBusy = false
