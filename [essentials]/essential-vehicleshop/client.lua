...
RegisterNetEvent("qb-vehicleshop:client:AdminFullyUpgradeCar", function()
    ...
    if IsPedInAnyVehicle(PlayerPedId()) then
        ...
        QBCore.Functions.Notify('Vehicle has been fully upgraded.')
    else
...
local function startTestDriveTimer(testDriveTime, shopname)
    ...
    CreateThread(function()-- Avoids the condition to run before entering vehicle
        while inTestDrive do
            if GetGameTimer() < gameTimer + tonumber(1000 * testDriveTime) then
                ...
                if (testDriveTime - secondsLeft / 1000) <= 1 or not IsPedInAnyVehicle(PlayerPedId()) then
                    ...
                    if IsPedInAnyVehicle(PlayerPedId()) then
                        ...
                        SetEntityCoords(PlayerPedId(), Config.Shops[shopname]['ReturnLocation'].x, Config.Shops[shopname]['ReturnLocation'].y, Config.Shops[shopname]['ReturnLocation'].z)
                    end
                    Wait(2000)
                    DoScreenFadeIn(2000)
                    Wait(2000)
                    ...
                    exports['ps-ui']:Notify(Lang:t('general.testdrive_complete'))
                    inTestDrive = false
                end
                if inTestDrive then
                    exports['ps-ui']:StatusShow(math.ceil(testDriveTime - secondsLeft / 1000), {
                        Lang:t('general.testdrive_timer'),
                    })
                end
...
RegisterNetEvent('qb-vehicleshop:client:customTestDrive', function(data)
    if not inTestDrive then
        ...
        QBCore.Functions.TriggerCallback('QBCore:Server:SpawnVehicle', function(netId)
            ...
            testDriveVeh = netId
            exports['ps-ui']:Notify(Lang:t('general.testdrive_timenoti', {testdrivetime = Config.Shops[tempShop]["TestDriveTimeLimit"]}))
        end, vehicle, Config.Shops[tempShop]["TestDriveSpawn"], false)
        createTestDriveReturn()
...
RegisterNetEvent('qb-vehicleshop:client:TestDriveReturn', function()
    ...
    if veh == entity then
        ...
        exports['ps-ui']:Notify(Lang:t('error.testdrive_return'), 'error')
    end
end)
...
RegisterCommand("fullupgrade", function()
    ...
    if inTestDrive then
        if invehicle then 
            if GetVehicleNumberPlateText(entity) == "TESTDRIV" then
                props = {}
                props.modEngine = 3
                props.modTransmission = 2
                props.modSuspension = -1
            ...
            exports['ps-ui']:Notify('You are not in a vehicle.')
        end
    else
        exports['ps-ui']:Notify('You are not in a test drive.')
    end
end)