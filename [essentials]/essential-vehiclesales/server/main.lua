...
QBCore.Functions.CreateCallback('essential-occasions:server:getVehicles', function(_, cb)
...
QBCore.Functions.CreateCallback("essential-occasions:server:getSellerInformation", function(_, cb, citizenid)
...
QBCore.Functions.CreateCallback("essential-vehiclesales:server:CheckModelName", function(_, cb, plate)
    if plate then
...
RegisterNetEvent('essential-occasions:server:ReturnVehicle', function(vehicleData)
    local src = source
    ...
    if result[1] then
        if result[1].seller == Player.PlayerData.citizenid then
            ...
            TriggerClientEvent("essential-occasions:client:ReturnOwnedVehicle", src, result[1])
...
RegisterNetEvent('essential-occasions:server:sellVehicle', function(vehiclePrice, vehicleData)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('essential-occasions:server:sellVehicleBack', function(vehData)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local price = 0
...
RegisterNetEvent('essential-occasions:server:buyVehicle', function(vehicleData)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local result = MySQL.query.await('SELECT * FROM occasion_vehicles WHERE plate = ? AND occasionid = ?',{vehicleData['plate'], vehicleData["oid"]})
    if result[1] and next(result[1]) then
        if Player.PlayerData.money.bank >= result[1].price then
            ...
            TriggerClientEvent("essential-occasions:client:BuyFinished", src, result[1])
            TriggerClientEvent('qb-occasion:client:refreshVehicles', -1)
