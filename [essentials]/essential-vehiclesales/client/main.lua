                ...
                if Config.UseTarget then
                    ...
                    EntityZones[i] = exports['qb-target']:AddTargetEntity(occasionVehicles[Zone][i].car, {
                        options = {
                            {
                                type = "client",
                                event = "essential-vehiclesales:client:OpenContract",
                                icon = "fas fa-car",
...
local function SellData(data, model)
    QBCore.Functions.TriggerCallback("essential-vehiclesales:server:CheckModelName",function(DataReturning)
        local vehicleData = {}
        ...
        TriggerServerEvent('essential-occasions:server:sellVehicle', data.price, vehicleData)
        ...
        while listen do
            if IsControlJustReleased(0, 38) then -- E
                if spot then
                    ...
                    TriggerEvent('essential-vehiclesales:client:OpenContract', data)
                else
                    if IsPedInAnyVehicle(PlayerPedId(), false) then
                        listen = false
                        TriggerEvent('essential-occasions:client:MainMenu')
                        --TriggerEvent('essential-vehiclesales:client:SellVehicle')
                    else
    ...
    for k, v in pairs(Config.Zones) do
        ...
        SellSpot:onPlayerInOut(function(isPointInside)
            if isPointInside and Zone ~= k then
                Zone = k
                QBCore.Functions.TriggerCallback('essential-occasions:server:getVehicles', function(vehicles)
...
RegisterNUICallback('buyVehicle', function(_, cb)
    TriggerServerEvent('essential-occasions:server:buyVehicle', CurrentVehicle)
    cb('ok')
end)

RegisterNUICallback('takeVehicleBack', function(_, cb)
    TriggerServerEvent('essential-occasions:server:ReturnVehicle', CurrentVehicle)
    cb('ok')
end)

-- Events

RegisterNetEvent('essential-occasions:client:BuyFinished', function(vehdata)
...
RegisterNetEvent('essential-occasions:client:SellBackCar', function()
    ...
    if IsPedInAnyVehicle(ped, false) then
        ...
        QBCore.Functions.TriggerCallback('qb-garage:server:checkVehicleOwner', function(owned, balance)
            if owned then
                if balance < 1 then
                    TriggerServerEvent('essential-occasions:server:sellVehicleBack', vehicleData)
...
RegisterNetEvent('essential-occasions:client:ReturnOwnedVehicle', function(vehdata)
    local vehmods = json.decode(vehdata.mods)
    DoScreenFadeOut(250)
    Wait(500)
    QBCore.Functions.TriggerCallback('QBCore:Server:SpawnVehicle', function(netId)
...
RegisterNetEvent('essential-vehiclesales:client:SellVehicle', function()
...
RegisterNetEvent('essential-vehiclesales:client:OpenContract', function(data)
    CurrentVehicle = occasionVehicles[Zone][data.Contract]
    if CurrentVehicle then
        QBCore.Functions.TriggerCallback('essential-occasions:server:getSellerInformation', function(info)
            if info then
...
RegisterNetEvent('essential-occasions:client:MainMenu', function()
    local MainMenu = {
        {
            isMenuHeader = true,
        ...
        },
        {
            ...
            params = {
                event = 'essential-vehiclesales:client:SellVehicle',
            }
        },
        {
            ...
            params = {
                event = 'essential-occasions:client:SellBackCar',
            }
        }
    }
