...
local function getThresholdRecipes(craftingRep, weaponreputation, attachmentRep, ammoreputation, benchType)
    ...
    for k, v in pairs(Config.defaultRecipes) do
        if v.benchType == benchType then
            ...
            for k, weaponRep in pairs(Config.benchReps.weaponreputation) do
                if v.benchType == weaponRep then
                    if weaponreputation >= v.threshold then
                        playerDefaultRecipes[#playerDefaultRecipes + 1] = v
                    end
                end
            end
            for k, attachRep in pairs(Config.benchReps.attachmentRep) do
             ...
             for k, ammoRep in pairs(Config.benchReps.ammoreputation) do
                if v.benchType == ammoRep then
                    if ammoreputation >= v.threshold then
                        playerDefaultRecipes[#playerDefaultRecipes + 1] = v
                    end
                end
            end
    ...
    if not objExists then
        ...
        exports['qb-target']:AddTargetEntity(object, {
            options = { {
                ...
                label = "Craft",
                action = function()
                    TriggerServerEvent("essential-crafting_sv:getWorkBenchData")
                end
            }
            },
            distance = 1.5
...
RegisterNUICallback("discardBlueprint", function(data, cb)
    if currentBenchId then
        QBCore.Functions.TriggerCallback('essential-crafting_sv:discardBlueprint', function (result)
            cb(result)
...
RegisterNUICallback("attemptCraft", function(data, cb)
    if currentBenchId then
        TriggerServerEvent("essential-crafting_sv:attemptCraft", currentBenchId, data.currentRecipe, data.amt, data.isBlueprintRecipe)
    end
end)
...
RegisterNetEvent("essential-crafting_cl:openCraftingBench", function(craftingBenchData, benchId)
    ...
    local weaponreputation = PlayerData.metadata.weaponmakingrep
    local ammoreputation = PlayerData.metadata.ammomakingrep
    ...
    if craftingBenchData then
        ...
        local defaultRecipes = getThresholdRecipes(craftingRep, weaponreputation, attachmentRep, ammoreputation, benchType)
...
RegisterNetEvent("essential-crafting_cl:increasedRep", function(craftingRep, weaponreputation, attachmentRep, ammoreputation, benchType)
    local recipes = getThresholdRecipes(craftingRep, weaponreputation, attachmentRep, ammoreputation, benchType)
