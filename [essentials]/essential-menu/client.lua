...
local inventoryName = 'qb-inventory' -- @essential: make sure script using correct name
...
RegisterNUICallback("dataPost", function(data, cb)
    ...
    -- @essential: added PlaySoundFrontend to play menu sfx
    ...
    if rData then
        ...
        if rData.action then
            -- @essential: added action to trigger a function
        ...
        if rData.event and Promise == nil then
            -- @essential: added qbcore/fivem command
...
ProcessParams = function(data)
    for _, v in pairs(data) do
        ...
        -- @essential: get images from user inventory
        if v.image then
...
RegisterNetEvent("essential-menu:createMenu", ContextMenu)
RegisterNetEvent("essential-menu:closeMenu", CancelMenu)
-- @essential: overlay
RegisterNetEvent("essential-menu:Overlay", Overlay)
RegisterNetEvent("essential-menu:closeOverlay", CloseOverlay)
    ...
    for key, item in pairs(menu) do
        ...
        if item.params then
            ...
            if temp_btn.event == 'qb-menu:closeMenu' then
                temp_btn.event = 'essential-menu:closeMenu'
            end
        end
