...
RegisterNetEvent("qb-rental:returnvehicle", function()
  ...
  if car ~= 0 then
    ...
    if string.find(tostring(plate), "ES") then
...
RegisterNetEvent("qb-rental:vehiclespawn", function(data, cb)
  ...
  QBCore.Functions.SpawnVehicle(model, function(veh)
    SetVehicleNumberPlateText(veh, "ESHub"..tostring(math.random(1000, 9999)))
...
--       local isRental = vin ~= nil and string.sub(vin, 2, 3) == "ES"
--       if isRental then
--         TaskWarpPedIntoVehicle(PlayerPedId(), veh, -1)
--         TriggerEvent("vehiclekeys:client:SetOwner", QBCore.Functions.GetPlate(veh))
...
exports['qb-target']:AddBoxZone("NewRentalMenu4", vector3(-1993.3, -332.21, 48.11), 1, 1.5, {
  name="NewRentalMenu4",
  heading=235,
  debugPoly=false,
  minZ=44.91,
