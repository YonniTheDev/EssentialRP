...
RegisterNetEvent('essential-dongle:activity', function()
            local header = {
                {
                    isMenuHeader = true,
...
RegisterNetEvent('essential-dongle:buyitems', function(data)
    local header = {
        {
            isMenuHeader = true,
            icon = "fa-solid fa-mask",    
            header = "Hmm what are these items?"
        }
    }
    for k, v in pairs(Config.Shop) do
        if QBCore.Shared.Items[v.item].label then
            header[#header+1] = {
                ...
                params = {
                    isServer = true,
                    event = "essential-dongle:server:buyshit",
                    args = k
                }
            }
        end
...
-- do something like when a TriggerClientEvent('essential-dongle:client:SetBankCD', -1, true) when a bank is being robbed
-- do something like TriggerClientEvent('essential-dongle:client:SetBankCD', -1, false) when a bank can be hit again

RegisterNetEvent('essential-dongle:client:SetBankCD', function(bool)
    BankRobberyCD = bool
end)
...
CreateThread(function()
    exports['qb-target']:AddTargetModel('cs_old_man2', {
        options = {
            { 
                type = "client",
                event = "essential-dongle:activity",
                icon = "fas fa-clock",
            ...
            {
                type = "client",
                event = "essential-dongle:buyitems",
