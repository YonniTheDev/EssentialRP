...
local function OpenMainMenu()
    ...
    Core.Functions.TriggerCallback('burglary:NotEnoughCopsOnline', function(enough)
        ...
        -----------------[[default main menu if rep is disabled]]------------------
        local mainMenu = {{
        ...
        if Config.Reputation then
            GetRepInfo(function(repData) -- Get current level info from cl_exp
        ...
        if not Config.Reputation then exports[Config.MenuResourceName]:openMenu(mainMenu) end
    end)
end
        ...
        while true do
            ...
            if #(plyPos - pedSpawn) < pedDistance then
                if buyerEntity == nil then
                    ...
                    buyerEntity = CreatePed(0, pedModel, pedSpawn.x, pedSpawn.y, pedSpawn.z, Config.Buyers[id]
                    ["heading"], true, false)
...
local function LockpickFinish(success)
    if success then
        ...
        TriggerServerEvent("burglary:server:SyncChance", InGroup and Groups[CurrentGroup].members or nil, math.random(1, 100)) -- Make sure the chance of the location spawning is synced
    else
        ClearPedTasks(PlayerPedId())
        RemoveLockpick()
    end
...
RegisterNetEvent("lockpicks:UseLockpickSquare", function()
    ...
    if not house[currentId]["unlocked"] then
        ...
        local Skillbar = exports["essential-skillbar"]:GetSkillbarObject()
        Skillbar.Start({
...
--- Add qb-target entity zone from cl_escrow.lua, this function will handle every entity other than safes
function AddEntityTarget(targetId, entity, rewardId, icon, displayName, type, hideHeading)
    ...
    exports[Config.TargetResourceName]:AddEntityZone(targetId, entity, {
        ...
        debugPoly = false,
        minZ = GetEntityCoords(entity).z - 25.0,
        maxZ = GetEntityCoords(entity).z + 3.0,
    }, {
        options = {
            {
                icon = icon,
                ...
                action = function()
                    if type == "pickup" then
                        PickupReward(rewardId)
                    elseif type == "key" then
                        PickupKey(rewardId)
                    elseif type == "hide" then
                        PeakabooMaafk(hideHeading)
                    end
                end,
            },
        },
...
--- Add qb-target entity zone (specifically for safes) from cl_escrow.lua, this function will only handle safe entities 
function AddEntitySafeTarget(targetId, entity, rewardId)
    -- You can add support for your own target resource here
    exports[Config.TargetResourceName]:AddEntityZone(targetId, entity, {
        name = targetId,
        heading = GetEntityHeading(entity),
        ...
        options = {
            {
                icon = "fa fa-key",
                label = Config.Prompts["safe1"],
                action = function()
                    SafeReward(rewardId, false) -- Find the key and unlock safe
                end,
            },
            {
                icon = "fa-solid fa-lock-open",
                label = Config.Prompts["safe2"],
                action = function()
                    local item = GetSafeItem()
                    if PlyHasItem(item, 1) then
                        CrackSafe(rewardId) -- Crack safe with the required item
                    else
                        QbNotify(Config.Prompts["bossman2"], "primary")
                    end
                end,
            },
        },
...
function AddBoxTarget(ids, placement, display, type, minMaxZ, size)
    local targetId = ids[1]
    local tableId = ids[2]

    local icon = display[1]
    local labelText = display[2]

    local pos = placement[1]
    local heading = placement[2]

    --print(ids, placement, display, type, minMaxZ, size)
        ...
        options = {
            {
                ...
                action = function()
                    if type == "security" then
                        local spawnId = ids[3]
                        DisableSecurity(tableId, spawnId)
                    elseif type == "enter" then
                        BreakOrEnter()
                    elseif type == "exit" then
                        TpToExterior()
                    elseif type == "search" then
                        SearchReward(tableId)
                    elseif type == "pickup" then
                        PickupReward(tableId)
                    elseif type == "hide" then
                        PeakabooMaafk(heading)
                        ResetHidingStuff(tableId)
                    end
                end,
            },
        },
