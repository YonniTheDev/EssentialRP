...
AddEventHandler('essential-hunting:client:slaughterAnimal', function(entity)
    ...
    CoreName.Functions.Progressbar("harv_anim", "Slaughtering Animal", Config.SlaughteringSpeed, false,
        ...
        if AnimalLootMultiplier:read(entity) ~= nil and AnimalLootMultiplier:read(entity) ~= false then
            TriggerServerEvent('essential-hunting:server:AddItem', animal, NetworkGetNetworkIdFromEntity(entity),
                AnimalLootMultiplier:read(entity))
        else
            -- defalut values for multipiler
            TriggerServerEvent('essential-hunting:server:AddItem', animal, NetworkGetNetworkIdFromEntity(entity), 'default')
        end
    end)
end)

AddEventHandler('essential-hunting:client:sellREQ', function()
    TriggerServerEvent('essential-hunting:server:sellmeat')
end)

RegisterNetEvent('essential-hunting:client:ForceRemoveAnimalEntity')
AddEventHandler('essential-hunting:client:ForceRemoveAnimalEntity', function(entity)
    DeleteEntity(entity)
    ...
    if outPosition.x ~= 0 and outPosition.y ~= 0 and outPosition.z ~= 0 then
        Citizen.CreateThread(function()
            ...
            if startSpawningTimer == 0 then
                ...
                TriggerServerEvent('essential-hunting:server:choiceWhichAnimalToSpawn', coord, outPosition, zone_type,
                    indicator)
            else
...
RegisterNetEvent('essential-hunting:client:useBait', function()
...
RegisterNetEvent('essential-hunting:client:spawnAnimal', function(coord, outPosition, C_animal, was_llegal, indicator)
    ...
    TriggerServerEvent('essential-hunting:server:removeBaitFromPlayerInventory')
...
RegisterNetEvent('essential-hunting:client:spawnanim', function(model, was_llegal)
...
RegisterNetEvent('essential-hunting:client:clearTask')
AddEventHandler('essential-hunting:client:clearTask', function()
    local playerPed = PlayerPedId()
