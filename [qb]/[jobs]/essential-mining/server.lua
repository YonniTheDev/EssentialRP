...
---Crafting
RegisterServerEvent('essential-mining:GetItem', function(data)
	local src = source
	...
	TriggerClientEvent("essential-mining:CraftMenu", src, data)
end)

RegisterServerEvent('essential-mining:MineReward', function()
...
RegisterServerEvent('essential-mining:CrackReward', function(cost)
	local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
--Stone Cracking Checking Triggers
--Command here to check if any stone is in inventory
RegisterServerEvent('essential-mining:WashReward', function(cost)
	local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    Player.Functions.RemoveItem('stone', cost)
...
RegisterServerEvent('essential-mining:PanReward', function()
	local src = source
    local Player = QBCore.Functions.GetPlayer(src)
	for i = 1, math.random(1,3) do
...
RegisterNetEvent("essential-mining:Selling", function(data)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('essential-mining:server:toggleItem', function(give, item, amount)
	local src = source
	if give == 0 or give == false then
		...
		else TriggerEvent("essential-mining:server:DupeWarn", item, src) end -- if not boot the player
	else
...
RegisterNetEvent("essential-mining:server:DupeWarn", function(item, newsrc)
