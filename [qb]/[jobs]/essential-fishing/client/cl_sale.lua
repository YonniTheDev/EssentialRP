...
RegisterNetEvent('essential-fishing:client:OpenSale', function()
    -- Anti spam
    ...
    for k, v in pairs(items) do
        if fish[v.name] then
            menu[#menu+1] = {
                ...
                params = {
                    event = "essential-fishing:client:SellFish",
                    args = v.name
                }
            }
        end
...
RegisterNetEvent('essential-fishing:client:SellFish', function(itemName)
    ...
    QBCore.Functions.TriggerCallback('essential-fishing:server:CanSell', function(result)
        if result then
            -- Anti spam
            if isSelling then return end
            isSelling = true
            ...
            -- Sell Fish
            TriggerServerEvent('essential-fishing:server:SellFish', itemName, tonumber(sellingAmount.amount))
...
CreateThread(function()
    ...
    exports['qb-target']:AddTargetEntity(ped, {
        options = {
            {
                type = "client",
                event = "essential-fishing:client:OpenSale",
