...
RegisterNetEvent('essential-deliveries:TakeJob', function ()
...
RegisterServerEvent('essential-deliveries:CreateDeliveryGroupJob', function()
    local PlayerId = source
    local Player = QBCore.Functions.GetPlayer(PlayerId)
    ...
    if group then
        if Config.JobNeeded then
            if Player.PlayerData.job.name == Config.JobName then
                TriggerEvent('essential-deliveries:startjob', PlayerId)
            else
...
function GetStoreInfo(source, group)
    ...
    if playerRep >= Config.DeliveryHighReputation then
        payment = Config.Shops[3].Payment
    end
    return coords, packages, reputation, payment
end

RegisterNetEvent('essential-deliveries:startjob', function (source)
    local PlayerId = source
    local group = exports['qb-phone']:GetGroupByMembers(PlayerId)
    ...
    if PlayerId == leader then
        ...
        TriggerClientEvent('essential-deliveries:createjobvehicle', PlayerId, Jobs[group]['DeliveryPackages'])
    else
        TriggerClientEvent('QBCore:Notify', PlayerId, 'You are not group leader', "error", 3000)
    end
end)

RegisterNetEvent('essential-deliveries:givegroupvehiclekeys', function (VehiclePlate, TruckNetworkId)
    local PlayerId = source
    local group = exports['qb-phone']:GetGroupByMembers(PlayerId)
    ...
    for i = 1, #members do
        ...
        TriggerClientEvent('essential-deliveries:addvehicletarget', members[i], Jobs[group]['DeliveryTruckId'])
        TriggerClientEvent('essential-deliveries:addstoretarget', members[i], Jobs[group]['StoreLocation'])
    end
...
RegisterServerEvent('essential-deliveries:TakePackages', function()
    local PlayerId = source
    local Player = QBCore.Functions.GetPlayer(PlayerId)
    local group = exports['qb-phone']:GetGroupByMembers(PlayerId)
    ...
    if #(coords - dcoords) < 25.0 then
        ...
        if Player.Functions.AddItem(randomItem[1], 1, false) then
            ...
            if Jobs[group]['CollectedPackages'] ==  Jobs[group]['DeliveryPackages'] then
                local members = exports['qb-phone']:getGroupMembers(group)
                for i=1, #members do
                    TriggerClientEvent('essential-deliveries:RemoveTarget', members[i])
                end
            end
        end
    else
        TriggerClientEvent('essential-deliveries:changevarible', PlayerId)
        TriggerClientEvent('QBCore:Notify', PlayerId, 'To far from location..', 'error', 3000)
    end
end)

RegisterNetEvent('essential-deliveries:DeliverPackage', function (item)
    local PlayerId = source
    local Player = QBCore.Functions.GetPlayer(PlayerId)
    local group = exports['qb-phone']:GetGroupByMembers(PlayerId)
    local job = exports["qb-phone"]:getJobStatus(group)
    ...
    if Jobs[group]['DeliveredPackages'] < Jobs[group]['DeliveryPackages'] then
        if Player.Functions.RemoveItem(item, 1, false) then
            ...
            if Jobs[group]['DeliveredPackages'] ==  Jobs[group]['DeliveryPackages'] then
                local leader = exports['qb-phone']:GetGroupLeader(group)
                TriggerClientEvent('essential-deliveries:canreturn', leader)
...
RegisterNetEvent('essential-deliveries:ReturnVehicleAndClean', function ()
    local PlayerId = source
    local group = exports['qb-phone']:GetGroupByMembers(PlayerId)
    local leader = exports['qb-phone']:GetGroupLeader(group)
    local job = exports["qb-phone"]:getJobStatus(group)
    ...
    if PlayerId == leader then
        ...
        local members = exports['qb-phone']:getGroupMembers(group)
        for i=1, #members do
            TriggerClientEvent('essential-deliveries:cleareverything', members[i])
        end
        TriggerClientEvent('essential-deliveries:deletevehicle', PlayerId)
        TriggerEvent('essential-deliveries:CollectPayment', PlayerId)
    else
        TriggerClientEvent('QBCore:Notify', PlayerId, 'You are not group leader', "error", 3000)
    end
end)

RegisterNetEvent('essential-deliveries:CollectPayment', function (source)
    local PlayerId = source
    local group = exports['qb-phone']:GetGroupByMembers(PlayerId)
    local members = exports['qb-phone']:getGroupMembers(group)
    local leader = exports['qb-phone']:GetGroupLeader(group)
...
RegisterNetEvent('essential-deliveries:canceldelivery', function ()
    local PlayerId = source
    local group = exports['qb-phone']:GetGroupByMembers(PlayerId)
    local leader = exports['qb-phone']:GetGroupLeader(group)
    local job = exports["qb-phone"]:getJobStatus(group)
...
RegisterNetEvent('essential-deliveries:checkreputation', function ()
    local PlayerId = source
    local Player = QBCore.Functions.GetPlayer(PlayerId)
