...
RegisterNetEvent('essential-garbage:client:clientChecks', function(data)
...
RegisterNetEvent('essential-garbage:client:OpenMainMenu', function()
    local menu = {
        {
    ...
    if doinJob then
        menu[#menu+1] = {
            ...
            params = {
                isServer = true,
                event = "essential-garbage:server:CollectCheck",
            }
        }
    else
        menu[#menu+1] = {
            ...
            icon = "fa-solid fa-truck-arrow-right",
            params = {
                event = "essential-garbage:client:RequestJob",
            }
        }
    end
...
local function TakeGarbage(entity)
    QBCore.Functions.TriggerCallback('essential-garbage:server:CanGrabPackage', function(result)
        if result then
            Looted[entity] = true
            ...
            looting = true
            QBCore.Functions.Progressbar('RenewedGarbage', 'Grabbing Garbage...', math.random(500, 1000), false, false, { -- Name | Label | Time | useWhileDead | canCancel
...
RegisterNetEvent('essential-garbage:client:NewPlace', function(location, NetID, plate)
...
RegisterNetEvent('essential-garbage:client:JobDone', function()
    if blip then RemoveBlip(blip) blip = nil end

    SetNewWaypoint(Config.PedLocation.x, Config.PedLocation.y)
end)

RegisterNetEvent('essential-garbage:client:ResetClient', function()
    if blip then RemoveBlip(blip) blip = nil end
    looting = false
    CachedNet = nil
    curLocation = nil
...
CreateThread(function()
    ...
    exports['qb-target']:AddGlobalVehicle({
        options = {
          {
            type = "client",
            event = "essential-garbage:client:DeliverPackage",
            icon = 'fas fa-box',
            ...
            canInteract = function(entity)
                ...
                if #(GetEntityCoords(PlayerPedId()) - vector3(curLocation.x, curLocation.y, curLocation.z)) > 350.0 then return false end
    ...
    exports['qb-target']:SpawnPed({
        ...
        target = {
            options = {
                {
                    event = "essential-garbage:client:OpenMainMenu",
...
RegisterNetEvent('essential-garbage:client:DeliverPackage', function()
    local ped = PlayerPedId()
    TaskTurnPedToFaceEntity(ped, NetToVeh(CachedNet), 500)
    Wait(500)
    DeliverAnim(ped)
    QBCore.Functions.Progressbar('RenewedGarbage33', 'Throwing Trash in Trashmaster', 500, false, false, { -- Name | Label | Time | useWhileDead | canCancel
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
        ...
        ClearPedTasks(ped)
        QBCore.Functions.TriggerCallback('essential-garbage:server:DeliverPackage', function(result)
            if result then
                DeleteEntity(prop)
                prop = nil
            end
