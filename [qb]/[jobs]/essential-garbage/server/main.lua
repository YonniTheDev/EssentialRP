...
local function SpawnVehicle(carType, location, group, coords)
    ...
    for i=1, #m do
        TriggerClientEvent("essential-garbage:client:NewPlace", m[i], location, NetID, plate)
    end
    return NetID, plate
end

RegisterNetEvent('essential-garbage:server:StartJob', function(run, coords)
    local src = source
...
RegisterNetEvent('essential-garbage:server:CollectCheck', function()
    local src = source
    ...
    if pay > 0 then
        for i=1, #m do
            if m[i] then
                ...
                TriggerClientEvent('essential-garbage:client:ResetClient', m[i])
            end
        end
    else
        for i=1, #m do
...
RegisterNetEvent('essential-garbage:server:MaterialTrade', function(material)
...
QBCore.Functions.CreateCallback("essential-garbage:server:DeliverPackage", function(source, cb, Shop)
    ...
    if CurrentRuns[group].packages <= 0 then
        ...
        if CurrentRuns[group].runsLeft <= 0 then
            local m = exports['qb-phone']:getGroupMembers(group)
            for i=1, #m do
                TriggerClientEvent('essential-garbage:client:JobDone', m[i])
            end
            ...
            local m = exports['qb-phone']:getGroupMembers(group)
            for i=1, #m do
                TriggerClientEvent('essential-garbage:client:NewPlace', m[i], location)
            end
...
QBCore.Functions.CreateCallback('essential-garbage:server:CanGrabPackage', function(source, cb)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
AddEventHandler('qb-phone:server:GroupDeleted', function(group, players)
    ...
    CurrentRuns[group] = nil
    for i=1, #players do
        TriggerClientEvent("essential-garbage:client:ResetClient", players[i])
    end
end)
