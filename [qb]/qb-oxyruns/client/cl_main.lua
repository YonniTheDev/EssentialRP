...
local CreateDropOff = function()
	hasDropOff = true
	TriggerEvent('qb-phone:client:CustomNotification', 'CURRENT', "Make your way to the drop-off..", 'fa-regular fa-gem', '#3480eb', 8000)
	dropOffCount += 1
	...
	dropOffArea:onPlayerInOut(function(isPointInside, point)
		if isPointInside then
			if oxyPed == nil then
				TriggerEvent('qb-phone:client:CustomNotification', 'CURRENT', "Make the delivery..", 'fa-regular fa-gem', '#3480eb', 8000)
...
local StartOxyrun = function()
	...
	started = true
	TriggerEvent('qb-phone:client:CustomNotification', 'CURRENT', "Wait for a new location..", "fa-regular fa-gem", '#3480eb', 8000)
	while started do
		Wait(4000)
...
RegisterNetEvent("qb-oxyruns:client:StartOxy", function()
	if started then return end
	local hasItem = QBCore.Functions.HasItem(Config.Requireditem, 5)
	if not hasItem then
		QBCore.Functions.Notify("Your missing items come back later...", "error")
		return end
...
RegisterNetEvent('qb-oxyruns:client:DeliverOxy', function()
	...
	local hasItem = QBCore.Functions.HasItem(Config.Requireditem, 5)
	if not hasItem then
		QBCore.Functions.Notify("You have nothing for me...", "error")
		return end
	...
	if #(GetEntityCoords(ped) - GetEntityCoords(oxyPed)) < 5.0 then
		...
		TriggerServerEvent("qb-oxyruns:removeBosschains")
		...
		options = {
			{
				...
				label = 'Start Jewelry Run ($'..Config.StartRunPayment..')',
			}
		},
		distance = 2.0
	})
