...
RegisterNetEvent('qb-oxyruns:server:Reward', function()
    ...
    if Player then
        -- Cash
        local cash = math.random(850, 1000)
        Player.Functions.AddMoney("cash", cash, "run-money")
        ...
        -- -- Jewelery Remove State
        local Jewelery = math.random(100)
        if Jewelery <= Config.RemoveChance then
            Player.Functions.RemoveItem(Config.RunItem, 5, false)
            TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items[Config.RunItem], "remove", 5)
        end
...
QBCore.Functions.CreateCallback('qb-oxyruns:server:StartOxy', function(source, cb)
    ...
    if Player.PlayerData.money.cash >= Config.StartRunPayment then
        local amount = GetCopCount()
        if amount >= Config.MinCops then
            Player.Functions.RemoveMoney('cash', Config.StartRunPayment, "oxy start")
            cb(true)
        else
        ...
        TriggerClientEvent('QBCore:Notify', src, "You don't have enough money to start a run..", "error",  3500)
        cb(false)
    end
end)


RegisterServerEvent('qb-oxyruns:removeBosschains')
AddEventHandler('qb-oxyruns:removeBosschains', function(plate, model, money)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    TriggerClientEvent('inventory:client:ItemBox', src,  QBCore.Shared.Items["10kgoldchain"], 'remove')
    Player.Functions.RemoveItem('10kgoldchain', 5, false, info)
end)
