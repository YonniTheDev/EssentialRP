...
RegisterNetEvent('qb-boombox:client:pickupBoombox', function()
    ...
    if not DoesEntityExist(obj) then
        ...
        exports['qb-inventory']:toggleItem(1, 'boombox')
        currentData = nil
    end
    Wait(500)
    ClearPedTasks(PlayerPedId())
    ...
    while true do
        ...
        if LocalPlayer.state['isLoggedIn'] then
            ...
            if DoesEntityExist(object) then
                ...
                if #(coords - objCoords) < 4 and boomboxplaced then
                    currentData = NetworkGetNetworkIdFromEntity(object)
                    if not nearboombox then
                        nearboombox = true
                        exports['qb-core']:DrawText('[E] PLAY MUSIC')
                    end
