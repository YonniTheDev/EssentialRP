...
AddEventHandler('qb-storerobbery:server:CheckItem', function()
    ...
    local ItemData = Player.Functions.GetItemByName("safecracker","screwdriverset")
...
RegisterServerEvent('qb-storerobbery:removeAdvancedLockpick')
AddEventHandler('qb-storerobbery:removeAdvancedLockpick', function(plate, model, money)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    TriggerClientEvent('inventory:client:ItemBox', src,  QBCore.Shared.Items["AdvancedLockpick"], 'remove')
    Player.Functions.RemoveItem('AdvancedLockpick', 1, false, info)
end)

RegisterServerEvent('qb-storerobbery:removesafecracker')
AddEventHandler('qb-storerobbery:removesafecracker', function(plate, model, money)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    TriggerClientEvent('inventory:client:ItemBox', src,  QBCore.Shared.Items["safecracker"], 'remove')
    Player.Functions.RemoveItem('safecracker', 1, false, info)
end)