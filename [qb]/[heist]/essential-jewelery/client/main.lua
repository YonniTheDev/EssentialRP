...
AddEventHandler('QBCore:Client:OnPlayerLoaded', function()
	QBCore.Functions.TriggerCallback('essential-jewelerycustom:server:GetJewelleryState', function(result)
...
AddEventHandler('QBCore:Client:OnPlayerUnload', function()
  for i = 1, #Config.Vitrines do
    if Config.Vitrines[i].isBusy then
      TriggerServerEvent('essential-jewelerycustom:server:SetVitrineState', false, i)
    end
  end
  removeBlips()
end)
...
AddEventHandler('onResourceStart', function(resource)
  ...
  TriggerServerEvent('essential-jewelerycustom:server:StoreHit', 'all', false)
  createBlips()
end)
...
AddEventHandler('essential-jewelerycustom:client:SmashCase', function(case)
  QBCore.Functions.TriggerCallback('essential-jewelerycustom:server:GetCops', function(cops)
        ...
        if cops >= Config.RequiredCops then
          if isStoreHit(case, false) or isStoreHacked() then
            ...
            QBCore.Functions.Progressbar('smash_vitrine', Lang:t('info.smashing_progress'), Config.WhitelistedWeapons[pedWeapon].timeOut, false, true, {
              ...
              TriggerServerEvent('essential-jewelerycustom:server:VitrineReward', case)
              TriggerServerEvent('essential-jewelerycustom:server:SetTimeout', case)
              ...
              TriggerServerEvent('essential-jewelerycustom:server:SetVitrineState', 'isBusy', false, case)
              smashing = false
              TaskPlayAnim(ped, animDict, 'exit', 3.0, 3.0, -1, 2, 0, 0, 0, 0)
            end)
            TriggerServerEvent('essential-jewelerycustom:server:SetVitrineState', 'isBusy', true, case)
...
AddEventHandler('essential-jewelerycustom:client:Thermite', function(store, door)
        ...
        if dist <= 1.5 then
          if QBCore.Functions.HasItem(not door and Config.DoorItem or Config.BackRoom.Item) then
            ...
            exports['ps-ui']:Thermite(function(success) -- success
              if success then
                if not door then TriggerServerEvent('essential-jewelerycustom:server:StoreHit', store, true) end
                ...
                Wait(4000)
                TriggerServerEvent('essential-jewelerycustom:server:RemoveDoorItem', door)
                ...
                TriggerEvent('essential-jewelerycustom:client:HackSuccess', store, door)
...
AddEventHandler('essential-jewelerycustom:client:HackSecurity', function(door)
  QBCore.Functions.TriggerCallback('essential-jewelerycustom:server:GetCops', function(cops)
    if not checkTime(Config.VangelicoHours.range.open, Config.VangelicoHours.range.close) then
      if Config.Skills.enabled then
          ...
          if QBCore.Functions.HasItem(Config.HackItem) then
            ...
            exports['ps-ui']:VarHack(function(success)
              if success then
                ...
                hacking = false
                TriggerServerEvent('essential-jewelerycustom:server:StoreHit', 'all', true)
                Wait(250)
                StopAnimTask(ped, animDict, anim, 8.0)
                DeleteEntity(tab)
                TriggerEvent('essential-jewelerycustom:client:HackSuccess')
                TriggerServerEvent('essential-jewelerycustom:server:RemoveBackitem')
                TriggerServerEvent('essential-jewelerycustom:server:HackReward')  
              else
                hacking = false
...
AddEventHandler('essential-jewelerycustom:client:HackSuccess', function(store, door)
    ...
    if isStoreHit(store, true)  and not isStoreHacked() then
      if not Config.OneStore then
        ...
        if Config.AutoLock then
          TriggerServerEvent('essential-jewelerycustom:server:ToggleDoorlocks', store, false, false)
          locked = false
        ...
        if Config.AutoLock then TriggerServerEvent('essential-jewelerycustom:server:ToggleDoorlocks', store, false, false) end
        locked = false
      ...
      if Config.AutoLock and not checkTime(Config.VangelicoHours.range.open, Config.VangelicoHours.range.close) then
        TriggerServerEvent('essential-jewelerycustom:server:ToggleDoorlocks', store, true, false)
        locked = true
      end
      TriggerServerEvent('essential-jewelerycustom:server:StoreHit', store, false)
    else
      if not Config.OneStore then 
        QBCore.Functions.Notify(Lang:t('success.hacked_threestore'), 'success')
        if Config.AutoLock then TriggerServerEvent('essential-jewelerycustom:server:ToggleDoorlocks', nil, false, true) end
        locked = false
        Wait(Config.Cooldown)
        if Config.AutoLock and not checkTime(Config.VangelicoHours.range.open, Config.VangelicoHours.range.close) then
          locked = true
          TriggerServerEvent('essential-jewelerycustom:server:ToggleDoorlocks', nil, true, true)
        end
        TriggerServerEvent('essential-jewelerycustom:server:StoreHit', 'all', false)
      else
  ...
  if Config.AutoLock then TriggerServerEvent('essential-jewelerycustom:server:ToggleDoorlocks', store, false, false, true) end
  Wait(Config.Cooldown - warningTimer)
  ...
  if Config.AutoLock and not checkTime(Config.VangelicoHours.range.open, Config.VangelicoHours.range.close) and not isStoreHacked() then
    TriggerServerEvent('essential-jewelerycustom:server:ToggleDoorlocks', store, true, false, true)
  end
end)

-------------------------------- EVENTS --------------------------------

RegisterNetEvent('essential-jewelerycustom:client:SetVitrineState', function(stateType, state, k)
...
RegisterNetEvent('essential-jewelerycustom:client:StoreHit', function(storeIndex, isHit)
      ...
      options = {
        {
          ...
          action = function()
            if validWeapon() then
              TriggerEvent('essential-jewelerycustom:client:SmashCase', k)
            else
      ...
      options = {
        {
          ...
          item = 'thermite',
          action = function()
            TriggerEvent('essential-jewelerycustom:client:Thermite', k)
          end
        }
      },
      ...
      options = {
        {
          ...
          action = function()
            if GetSelectedPedWeapon(ped) == `WEAPON_UNARMED` then
              ...
              if validWeapon() then
                TriggerEvent('essential-jewelerycustom:client:SmashCase', i)
              else
                QBCore.Functions.Notify(Lang:t('error.wrong_weapon'), 'error')
              end
            end
    ...
    options = {
      {
        ...
        action = function()
          TriggerEvent('essential-jewelerycustom:client:Thermite', 1, false)
        end
      }
    },
    distance = 2.5 -- This is the distance for you to be at for the target to turn blue, this is in GTA units and has to be a float value
  ...
  options = {
    {
      ...
      action = function()
        TriggerEvent('essential-jewelerycustom:client:Thermite', 1, true)
      end
    }
  },
  distance = 2.5 -- This is the distance for you to be at for the target to turn blue, this is in GTA units and has to be a float value
...
}, 
{
  options = {
    ...
    action = function()
        TriggerEvent('essential-jewelerycustom:client:HackSecurity')
      end,
    canInteract = function()
    ...
    if LocalPlayer.state.isLoggedIn then
      if not checkTime(Config.VangelicoHours.range.open, Config.VangelicoHours.range.close) then
        ...
        if not loopDone then
          Wait(1000)
          TriggerServerEvent('essential-jewelerycustom:server:ToggleDoorlocks', nil, false, true, nil, true)
          loopDone = true
        end
      end
    end
