...
function LockpickDoor(isAdvanced)
    ...
    exports['ps-ui']:Circle(function(success)
        if success then
            ...
            exports['ps-ui']:Notify("You failed lockpick the vehicle !", "error")
        end
...
function lockpickFinish(success)
    ...
    if success then
        ...
        if GetPedInVehicleSeat(vehicle, -1) == PlayerPedId() then
            ...
            exports['ps-ui']:Notify(Lang:t("notify.vlockpick"), 'success')
    ...
    QBCore.Functions.Progressbar("rob_keys", Lang:t("progress.acjack"), Config.CarjackingTime, false, true, {
        ...
        if hasWeapon and isCarjacking then
            ...
            if random <= 7 then
                ...
                TriggerEvent("MakePedFlee",target)
                TriggerServerEvent('hud:server:GainStress', math.random(1, 4))
            end
            FreezeEntityPosition(vehicle, false)
            isCarjacking = false
